...
pub const PLUGIN_ID: &str = "notifier.teams";
...
impl TeamsPlugin {
    pub fn manifest_fn() -> PluginManifest {
        PluginManifest {
            ...
            description: "Microsoft Teams Adaptive Card webhook notifier.",
            kind: PluginKind::Notifier,
            author: "HiveGuard",
            docs_url: Some(
                "https://github.com/anthropics/hiveguard/blob/main/plugins/notifier-teams/README.md",
            ),
        }
    }
    ...
    pub fn create(
        ...
        Box::pin(async move {
            let mut plugin = TeamsPlugin {
                manifest: Self::manifest_fn(),
                state: RwLock::new(None),
            };
            <TeamsPlugin as Plugin>::init(&mut plugin, cfg).await?;
...
#[async_trait]
impl NotifierPlugin for TeamsPlugin {
...
inventory::submit! {
    PluginDescriptor {
        ...
        manifest: TeamsPlugin::manifest_fn,
        config_schema: include_str!("../schema.json"),
        factory: PluginFactory::Notifier(TeamsPlugin::create),
    }
}
...
mod tests {
    ...
    async fn factory_accepts_minimal() {
        ...
        let _p = TeamsPlugin::create(test_ctx(), cfg).await.unwrap();
    }

    #[tokio::test]
    async fn factory_rejects_missing_url() {
        match TeamsPlugin::create(test_ctx(), serde_json::json!({})).await {
    ...
    fn format_renders_ip_ban() {
        ...
        let out = TeamsPlugin::format(&event);
