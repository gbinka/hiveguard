...
use hiveguard_enforce::NftablesEnforcer;
use hiveguard_plugin_api::prelude::*;

const PLUGIN_ID: &str = "enforcer.nftables";
...
pub struct NftablesPlugin {
    manifest: PluginManifest,
    inner: Option<NftablesEnforcer>,
}
...
impl NftablesPlugin {
    pub fn manifest_fn() -> PluginManifest {
        PluginManifest {
            ...
            description: "nftables firewall enforcer.",
            kind: PluginKind::Enforcer,
            author: "HiveGuard",
            docs_url: Some("https://github.com/anthropics/hiveguard/blob/main/plugins/enforcer-nftables/README.md"),
        }
    }
    ...
    pub fn create(
        ...
        Box::pin(async move {
            let mut plugin = NftablesPlugin {
                manifest: Self::manifest_fn(),
                inner: None,
            };
            <NftablesPlugin as Plugin>::init(&mut plugin, cfg).await?;
            Ok(Box::new(plugin) as Box<dyn EnforcerPlugin>)
        })
    }

    fn inner_mut(&mut self) -> PluginResult<&mut NftablesEnforcer> {
        self.inner
            .as_mut()
            .ok_or_else(|| PluginError::Runtime("plugin used before init".into()))
    }

    fn inner_ref(&self) -> PluginResult<&NftablesEnforcer> {
        self.inner
            .as_ref()
            .ok_or_else(|| PluginError::Runtime("plugin used before init".into()))
    }
...
#[async_trait]
impl Plugin for NftablesPlugin {
    ...
    async fn init(&mut self, cfg: serde_json::Value) -> PluginResult<()> {
        ...
        self.inner = Some(NftablesEnforcer::new(
            parsed.table,
            parsed.set_name,
            Duration::from_secs(parsed.batch_interval_secs),
        ));
        Ok(())
    }
}
...
#[async_trait]
impl EnforcerPlugin for NftablesPlugin {
...
inventory::submit! {
    PluginDescriptor {
        ...
        manifest: NftablesPlugin::manifest_fn,
        config_schema: include_str!("../schema.json"),
        factory: PluginFactory::Enforcer(NftablesPlugin::create),
    }
}
...
mod tests {
    ...
    fn manifest_has_correct_id_and_kind() {
        let manifest = NftablesPlugin::manifest_fn();
    ...
    async fn factory_accepts_valid_config() {
        ...
        let _plugin = NftablesPlugin::create(test_ctx(), cfg).await.unwrap();
    }

    #[tokio::test]
    async fn factory_rejects_invalid_config() {
        let bad_cfg = serde_json::json!({ "batch_interval_secs": 1 });
        match NftablesPlugin::create(test_ctx(), bad_cfg).await {
