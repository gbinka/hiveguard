...
use hiveguard_core::detectors::DistributedSlowDetector;
use hiveguard_core::models::{EventType, NormalizedEvent};
use hiveguard_plugin_api::prelude::*;
use serde::Deserialize;

pub const PLUGIN_ID: &str = "detector.distributed_slow";
pub const PLUGIN_VERSION: &str = "0.1.0";

#[derive(Debug, Deserialize, Default)]
struct Config {
    window_secs: Option<u64>,
    subnet_threshold: Option<usize>,
    ban_duration_secs: Option<u64>,
    ban_scope: Option<String>,
}

pub struct DistributedSlowPlugin {
    manifest: PluginManifest,
    detector: DistributedSlowDetector,
}

impl DistributedSlowPlugin {
    ...
    pub fn create(
        ...
        Box::pin(async move {
            let mut plugin = DistributedSlowPlugin {
                manifest: Self::manifest_fn(),
                detector: DistributedSlowDetector::new(),
            };
            <DistributedSlowPlugin as Plugin>::init(&mut plugin, cfg).await?;
            let _ = ctx;
            Ok(Box::new(plugin) as Box<dyn DetectorPlugin>)
        })
    }

    fn from_config(cfg: Config) -> DistributedSlowDetector {
        let _ban_scope = cfg.ban_scope.unwrap_or_else(|| "/24".to_string());
        DistributedSlowDetector::with_config(
            Duration::from_secs(cfg.window_secs.unwrap_or(600)),
            cfg.subnet_threshold.unwrap_or(5),
...
#[async_trait]
impl Plugin for DistributedSlowPlugin {
...
impl DetectorPlugin for DistributedSlowPlugin {
...
inventory::submit! {
    PluginDescriptor {
        ...
        manifest: DistributedSlowPlugin::manifest_fn,
        config_schema: include_str!("../schema.json"),
        factory: PluginFactory::Detector(DistributedSlowPlugin::create),
    }
}
...
mod tests {
    ...
    async fn factory_accepts_defaults() {
        let p = DistributedSlowPlugin::create(test_ctx(), serde_json::json!({})).await;
        assert!(p.is_ok());
    }

    #[tokio::test]
    async fn factory_accepts_full_config() {
        let p = DistributedSlowPlugin::create(
            test_ctx(),
            serde_json::json!({
                "window_secs": 900,
                "subnet_threshold": 7,
    ...
    async fn process_returns_none_for_unrelated_event() {
        let p = DistributedSlowPlugin::create(test_ctx(), serde_json::json!({}))
            .await
            .unwrap();
