...
pub const PLUGIN_ID: &str = "notifier.discord";
...
impl DiscordPlugin {
    pub fn manifest_fn() -> PluginManifest {
        PluginManifest {
            ...
            author: "HiveGuard",
            docs_url: Some(
                "https://github.com/anthropics/hiveguard/blob/main/plugins/notifier-discord/README.md",
            ),
        }
    }
    ...
    pub fn create(
        ...
        Box::pin(async move {
            let mut plugin = DiscordPlugin {
                manifest: Self::manifest_fn(),
                state: RwLock::new(None),
            };
            <DiscordPlugin as Plugin>::init(&mut plugin, cfg).await?;
...
#[async_trait]
impl NotifierPlugin for DiscordPlugin {
...
inventory::submit! {
    PluginDescriptor {
        ...
        manifest: DiscordPlugin::manifest_fn,
        config_schema: include_str!("../schema.json"),
        factory: PluginFactory::Notifier(DiscordPlugin::create),
    }
}
...
mod tests {
    ...
    #[test]
    fn manifest_correct() {
        let m = DiscordPlugin::manifest_fn();
    ...
    async fn factory_accepts_minimal() {
        ...
        let _p = DiscordPlugin::create(test_ctx(), cfg).await.unwrap();
    }

    #[tokio::test]
    async fn factory_rejects_missing_url() {
        match DiscordPlugin::create(test_ctx(), serde_json::json!({})).await {
    ...
    fn format_renders_ip_ban() {
        ...
        let out = DiscordPlugin::format(&event);
